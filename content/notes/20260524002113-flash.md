+++
title = "Flash"
date = 2026-05-24
updated= 2026-06-07
+++

Flash is cool. Very cool. But also kind of confusing, but maybe that is just a me thing. Anyways, flash storage has five organizational levels. From largest to smallest these are

1.  package: the entire chip itself
2.  die: an independent unit capable of executing commands, a package has at least one
3.  planes: a die can have one or two (sometimes more) planes, each plane
    performs the same actions concurrently
4.  blocks: a plane contains blocks, **the smallest unit which can be erased**
5.  pages: and blocks contains pages, **the smallest unit which can be programmed**

Note: it seems sometimes blocks might be referred to as sectors? Reading and writing (programming) operates on pages while erasing operates on blocks. Some flashes can also support **offsets** (sometimes called **columns**) into the pages, so you can write in the middle of the page if you want. Also it seems that there are often two stages for reading and programming

-   reading: load to cache, read from cache
-   programming: load program, execute program

The first step always involves writing or reading to some buffer first and the second step actually performs the process, whether it is making the data available after reading or actually storing the data onto the drive for flash. These seem to be a good way to cache up larger reads and writes, which should be better for performance.

And to erase data, you must do so on an entire block, which is larger than a page. This is problematic because you obviously should not erase an entire block each time you want to change a page, which is like eating one noodle with a giant fork. So instead the smarter option is to use a FTL (Flash Translation Layer), which keeps tracks of invalid pages so they can be erased later once the block is mostly filled with invalid data and to write the new data else where.

This is really useful for wear leveling as blocks have a limited **program erase cycle** (PE cycle) so by delaying the erase until it is really needed, we can extend the life of the flash, which is especially so in an embedded environment where such flashes have a much shorter lifecycle than their desktop counterparts. I find [LittleFS](@/notes/20260524010917-littlefs.md) to be a pretty good choice for embedded use.

Note: an empty NAND flash has all 1s. I think for flash there is no really a significant difference between blocks and sectors. Though I am not quite sure. See the following [link](https://stackoverflow.com/questions/12345804/difference-between-blocks-and-sectors).

There are however differences between Nand flash and Nor flash. Namely

| Nand Flash                   | Nor Flash                   |
|------------------------------|-----------------------------|
| faster writes and erases     | faster reads                |
| larger block sizes           | smaller block sizes         |
| higher standby current usage | lower standby current usage |
| higher PE cycle counts       | lower PE cycle counts       |

In particular, Nand flashes need much higher PE cycles because it is mandatory to perform an erase before a write. Indexing methods are different for both types, see [LittleFS](@/notes/20260524010917-littlefs.md) for how to index them.

Nor is better for cold storage while Nand is better for a general purpose storage due to its higher memory density. See [Flash Access](@/notes/20260607180622-flash_access.md) for in-depth information on how the cells are structured.
